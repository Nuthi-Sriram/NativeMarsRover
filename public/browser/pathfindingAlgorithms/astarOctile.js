function astarOctile(nodes, start, target, nodesToAnimate, boardArray, name, heuristic) {
...
function updateNode(currentNode, targetNode, actualTargetNode, name, nodes, actualStartNode, heuristic, boardArray) {
  ...
  if (!targetNode.heuristicDistance) targetNode.heuristicDistance = octile(targetNode, actualTargetNode);
...
function octile(nodeOne, nodeTwo) {
  ...
  var F = Math.SQRT2 - 1;
  return (xChange < yChange) ? F * xChange + yChange : F * yChange + xChange;
}



module.exports = astarOctile;
